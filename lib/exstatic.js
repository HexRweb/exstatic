...
class Exstatic {
	...
	initializeProperties(opts) {
		...
		this.docs = false;
		this.data = {};
		this._hbs = new HandlebarsCompiler(this);
	}
	...
	loadHelpers() {
		...
		return this.hook.executeHook('pre-register_helpers', resolverArgs).then(helpers => {
			['sync', 'async'].forEach(helperType => {
				Object.keys(helpers[helperType]).forEach(helper => {
					log.debug(t('Exstatic.registering_helper', {
						type: helperType,
						name: helper
					}));
					this._hbs.executeRegisterHelper(
						helperType, helper, helpers[helperType][helper]
					);
				});
			});
	...
	initialize(overrides = {}) {
		...
		return Promise.all(this.loadHelpers(), this.loadFileTypes());
	}
