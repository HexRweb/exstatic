...
class Exstatic {
	...
	loadFiles() {
		...
		return generateFileList(this.inputDir, blacklist).map(file => {
			const theFile = new File(file, this.inputDir, this.data.site.url);
	...
	write() {
		...
		return Promise.mapSeries([this.docs.core, this.docs.files], fileList =>
			Promise.mapSeries(fileList, file => {
				...
				return file.compileSection(this.compileProxy.bind(this), this.tempDir).then(() =>
					file.buildMarkdown()
				).then(() =>
					file.compileFile(this.compileProxy.bind(this), this.tempDir)
				);
